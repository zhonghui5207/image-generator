    ...
    try {
      ...
      if (useStream) {
        ...
        try {
          ...
          // 扣除用户积分 - 根据生成结果调整积分消耗
          const actualCreditsToUse = isGenerationFailed ? 1 : creditsToUse; // 如果生成失败，只我1积分
        ...
        if (!generatedImageUrl) {
          ...
          try {
            ...
            // 上传图片二进制内容到OSS
            const uploadResult = await ossClient.client.put(ossPath, imageBuffer);
